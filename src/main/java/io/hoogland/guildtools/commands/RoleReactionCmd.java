...
public class RoleReactionCmd extends Command {
    ...
    private SimpleDateFormat formatter = new SimpleDateFormat("hh:mm:ss dd-M-yyyy");
        ...
        if (event.getMember().hasPermission(Permission.MANAGE_ROLES) && validCommand) {
            if (event.getSelfMember().hasPermission(Permission.MANAGE_ROLES) &&
                ...
                MessageEmbed embed = EmbeddedUtils
                        .buildReactionRoleEmbed(EmbedConstants.ADD_REACT_TITLE_1, EmbedConstants.ADD_REACT_DESCRIPTION_CHANNEL, null, null);

                event.getMessage().delete().queue();

                Message message = event.getChannel().sendMessage(embed).complete();
                ReactionRole role = new ReactionRole(event.getGuild().getId());

                waitForChannelId(event, message, role);
            } else {
                MessageEmbed embed = EmbeddedUtils.buildReactionRoleEmbed(EmbedConstants.ADD_REACT_INSUFFICIENT_PERMISSIONS_TITLE,
                        EmbedConstants.ADD_REACT_DESCRIPTION__INSUFFICIENT_PERMISSIONS, null, "Insufficient Permissions");
                execute(event);
            }
    ...
    private void cancelCommand(@NotNull Message userMessage, @NotNull Message embeddedMessage) {
        MessageEmbed embed = EmbeddedUtils.buildReactionRoleEmbed(EmbedConstants.ADD_REACT_TITLE_CANCEL,
                null, null, EmbedConstants.CANCEL_ERROR, "Cancelled at " + formatter.format(new Date()));
    ...
    private void waitForChannelId(CommandEvent event, Message message, ReactionRole role) {
        waiter.waitForEvent(GuildMessageReceivedEvent.class,
                ...
                e -> {
                    if (e.getMessage().getContentRaw().equalsIgnoreCase(EmbedConstants.CANCEL_COMMAND)) {
                        cancelCommand(e.getMessage(), message);
                    } else {
                        if (!e.getMessage().getMentionedChannels().isEmpty()) {
                            role.getReactionRoleMessage().setChannelId(e.getMessage().getContentRaw());
                            MessageEmbed embed = EmbeddedUtils.buildReactionRoleEmbed(EmbedConstants.ADD_REACT_TITLE_2,
                                    EmbedConstants.ADD_REACT_DESCRIPTION_MESSAGE, role, null);
                            message.editMessage(embed).queue();
                            e.getMessage().delete().queue();

                            waitForMessageId(event, message, role);
                        } else {
                            MessageEmbed embed = EmbeddedUtils.buildReactionRoleEmbed(EmbedConstants.ADD_REACT_TITLE_1,
                                    EmbedConstants.ADD_REACT_DESCRIPTION_CHANNEL, null, "No channel found in your response.");
                            message.editMessage(embed).queue();
                            e.getMessage().delete().queue();
                            waitForChannelId(event, message, role);
                        }
        ...
        waiter.waitForEvent(GuildMessageReceivedEvent.class,
                ...
                e -> {
                    if (e.getMessage().getContentRaw().equalsIgnoreCase(EmbedConstants.CANCEL_COMMAND)) {
                        ...
                        event.getGuild().getTextChannelById(role.getReactionRoleMessage().getChannelIdSanitized())
                                .retrieveMessageById(Long.parseLong(e.getMessage().getContentRaw())).queue(
                                success -> {

                                    /*
                                     * todo search database for matches based on message ID and channel ID.
                                     *  if it doesn't exist, keep new. Don't save yet in case of cancel.
                                     */
                                    Optional<ReactionRoleMessage> optionalReactionRoleMessage = reactionRoleMessageRepository
                                            .findDistinctByMessageIdAndChannelIdAndGuildId(e.getMessage().getContentRaw(),
                                                    role.getReactionRoleMessage().getChannelId(), message.getGuild().getId());
                                    if (optionalReactionRoleMessage.isPresent()) {
                                        ...
                                        role.getReactionRoleMessage().setMessageId(e.getMessage().getContentRaw());
                                    ...
                                    MessageEmbed embed = EmbeddedUtils.buildReactionRoleEmbed(EmbedConstants.ADD_REACT_TITLE_3,
                                            EmbedConstants.ADD_REACT_DESCRIPTION_EMOJI, role, null);
                                    message.editMessage(embed).queue();
                                    e.getMessage().delete().queue();
                                ...
                                failure -> {
                                    MessageEmbed embed = EmbeddedUtils.buildReactionRoleEmbed(EmbedConstants.ADD_REACT_TITLE_1,
                                            EmbedConstants.ADD_REACT_DESCRIPTION_CHANNEL, null,
                                            String.format(EmbedConstants.MESSAGE_ERROR, failure.getMessage()));
                                    message.editMessage(embed).queue();
                                    e.getMessage().delete().queue();
                                    waitForMessageId(event, message, role);
                                });
                ...
                e -> {
                    if (e.getMessage().getContentRaw().equalsIgnoreCase(EmbedConstants.CANCEL_COMMAND)) {
                        ...
                        if (discordEmoteList.size() + regularEmoteList.size() != 1) {
                            ...
                            MessageEmbed embed = EmbeddedUtils.buildReactionRoleEmbed(EmbedConstants.ADD_REACT_TITLE_3,
                                    EmbedConstants.ADD_REACT_DESCRIPTION_EMOJI, null,
...
//                            waitForEmoji(event, message, role);
                            ...
                            String emoji = discordEmoteList.isEmpty() ?
                                    ...
                                    .format(EmbedConstants.MENTION_EMOJI, discordEmoteList.get(0).getName(), discordEmoteList.get(0).getId());
                            ...
                            if (!role.getReactionRoleMessage().getRoles().isEmpty()) {
                                ...
                                if (inUse) {
                                    ...
                                    MessageEmbed embed = EmbeddedUtils.buildReactionRoleEmbed(EmbedConstants.ADD_REACT_TITLE_3_EDIT,
                                            EmbedConstants.ADD_REACT_EMOJI_DUPLICATE, role, null);
                                    message.editMessage(embed).queue();
                            ...
                            MessageEmbed embed = EmbeddedUtils.buildReactionRoleEmbed(EmbedConstants.ADD_REACT_TITLE_4,
                                    EmbedConstants.ADD_REACT_DESCRIPTION_ROLE, role, null);
                            message.editMessage(embed).queue();
                    ...
                    if (e.getMessage().getContentRaw().equalsIgnoreCase(EmbedConstants.CANCEL_COMMAND)) {
                        ...
                        if (e.getMessage().getMentionedRoles().size() != 1) {
                            ...
                            if (canInteract) {
                                ...
                                role.setRoleId(e.getMessage().getMentionedRoles().get(0).getId());
                                System.out.println(e.getMessage().getMentionedRoles().get(0).getId());

                                MessageEmbed embed = EmbeddedUtils.buildReactionRoleEmbed(EmbedConstants.ADD_REACT_TITLE_5,
                                        EmbedConstants.ADD_REACT_DESCRIPTION_TYPE, role, null);
                                message.editMessage(embed).queue();
                ...
                e -> {
                    if (e.getMessage().getContentRaw().equalsIgnoreCase(EmbedConstants.CANCEL_COMMAND)) {
                        ...
                        try {
                            ...
                            MessageEmbed embed = EmbeddedUtils.buildReactionRoleEmbed(EmbedConstants.ADD_REACT_TITLE_SUMMARY,
                                    EmbedConstants.ADD_REACT_DESCRIPTION_SUMMARY, role, null,
                                    "Reaction role created at " + formatter.format(new Date()));
                            message.editMessage(embed).queue();
                            e.getMessage().delete().queue();
                            ...
                            MessageEmbed embed = EmbeddedUtils.buildReactionRoleEmbed(EmbedConstants.ADD_REACT_TITLE_5,
                                    EmbedConstants.ADD_REACT_DESCRIPTION_TYPE, null, "Please select either 1 or 2.");
                            message.editMessage(embed).queue();
                            e.getMessage().delete().queue();
                            waitForType(event, message, role);
                        }
        ...
        waiter.waitForEvent(GuildMessageReceivedEvent.class,
                ...
                e -> {
                    if (e.getMessage().getContentRaw().equalsIgnoreCase(EmbedConstants.CANCEL_COMMAND + "!")) {
                        ...
                        MessageEmbed embed = EmbeddedUtils.buildReactionRoleEmbed(EmbedConstants.ADD_REACT_TITLE_4,
                                EmbedConstants.ADD_REACT_DESCRIPTION_ROLE, duplicate, null);
                        message.editMessage(embed).queue();
                        e.getMessage().delete().queue();
    ...
    private void addEmoji(CommandEvent event, Message message, ReactionRole role) {
        Message msgWithReaction = event.getJDA().getGuildById(role.getReactionRoleMessage().getGuildId())
                .getTextChannelById(role.getReactionRoleMessage().getChannelIdSanitized())
        ...
        msgWithReaction.addReaction(role.getEmojiIdSanitized()).queue(
                ...
                failure -> {
                    MessageEmbed embed = EmbeddedUtils.buildReactionRoleEmbed(EmbedConstants.ADD_REACT_TITLE_SUMMARY,
                            EmbedConstants.ADD_REACT_DESCRIPTION_SUMMARY, role,
